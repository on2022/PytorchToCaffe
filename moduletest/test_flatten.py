...
def testflatten():
    ...
    caffeoutput = caffeblobs['flatten_blob1'].data[0][...].flatten()
    ...
    print('%s torchtime: %f caffetime: %f pytorch_shape: %-10s caffe_shape: %-10s output_mean_diff: %.10f' % ('flatten_blob1', torchtime, caffetime, torchoutput.shape, caffeoutput.shape, diff/torchoutput.size))
